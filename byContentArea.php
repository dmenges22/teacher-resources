...
<body>
		...
		if(!empty($_POST['type'])){
		 ...
		 showerror();
			$select=$_POST["content"];
			$query= "SELECT nameOfSite, description, linkToSite, image, altImageURL, comment, teachingEnvironment, pricingModel
			FROM ResourceInfo, SubjectInfo, Rating, TeacherComments, Keyword WHERE contentArea = content AND resourceInfoID = subjectInfoID AND 
			AND ratingID = subjectInfoID AND teacherCommentsID = subjectInfoID;";

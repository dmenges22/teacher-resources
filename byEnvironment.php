...
<body>
		...
		if(!empty($_POST['type'])){
		 ...
		 showerror();
				$select=$_POST["environment"];
				$query= "SELECT nameOfSite, description, linkToSite, image, altImageURL, comment, teachingEnvironment, pricingModel
			FROM ResourceInfo, SubjectInfo, Rating, TeacherComments, Keyword WHERE teachingEnvironment = environment AND resourceInfoID = SubjectInfoID AND ratingID = subjectInfoID AND teacherCommentsID = subjectInfoID";
